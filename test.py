...
class DBTestCase(unittest.TestCase):
    ...
    def test_error_message_table_full(self):
        ...
        '''
        fill_stdin = [
            'insert {0} user{0} person{0}@example.com'.format(index)
    ...
    def test_allows_inserting_strings_of_maximum_length(self):
        ...
        long_email = 'a' * 255
        script = [
            'insert 1 {} {}'.format(long_username, long_email), 'select',
            '.exit'
        ]
        self.assertEqual(
            self.run_command(script), [
                'db > Executed.', 'db > (1, {}, {})'.format(
                    long_username, long_email), 'Executed.', 'db > '
            ])
